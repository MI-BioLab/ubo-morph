...
from ubo_morph.device import SUPPORTED_DEVICES
from ubo_morph.landmarks import (
    DlibLandmarkExtractor,
    LandmarkExtractor,
    MediaPipeLandmarkExtractor,
)
from ubo_morph.morphing import MorphResult, morph_with_landmarks
from ubo_morph.utils import ensure_bgr_uint8


_IMAGE1_HEADERS = {"image1", "image_1"}
_IMAGE2_HEADERS = {"image2", "image_2"}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    morph_group.add_argument("--device", choices=SUPPORTED_DEVICES, default="cpu")
...
def _cli_factor_pairs(
    *,
    ...
    if factor is not None and (warping_factors is not None or blending_factors is not None):
        raise CliError("--factor cannot be combined with --warping-factor or --blending-factor")
...
def _read_pairs_csv(path: Path) -> tuple[list[PairSpec], bool]:
    ...
    table = table.fillna("").map(lambda value: str(value).strip())
    ...
    if not has_header:
        ...
        for row in table.itertuples(index=False, name=None):
            ...
            image1 = Path(row[0])
            image2 = Path(row[1])
            if not image1.is_absolute():
                image1 = path.parent / image1
            if not image2.is_absolute():
                image2 = path.parent / image2
            output_name = None
            if column_count == 3:
                if not row[2]:
    ...
    if has_linked_factor and (has_warping_factor or has_blending_factor):
        raise CliError("CSV factor cannot be combined with warping_factor or blending_factor")
    if has_warping_factor != has_blending_factor:
        raise CliError("CSV must contain both warping_factor and blending_factor columns")
    ...
    for row_number, row in enumerate(table.itertuples(index=False, name=None), start=2):
        ...
        image1 = Path(values[image1_column])
        image2 = Path(values[image2_column])
        if not image1.is_absolute():
            image1 = path.parent / image1
        if not image2.is_absolute():
            image2 = path.parent / image2
        ...
        if output_column is not None:
            if not values[output_column]:
                raise CliError(f"CSV output filenames cannot be empty (row {row_number})")
...
def _build_jobs(
    ...
    *,
    ...
    for pair in pairs:
        ...
        if pair.output_name is not None and len(factors) > 1 and not intermediate_results:
            raise CliError(
        ...
        for warping_factor, blending_factor in factors:
            if intermediate_results:
                output_path = (
                    output_dir
                    / f"M_{pair.image1.stem}_{pair.image2.stem}"
                    / "morphed.png"
                )
...
def _default_output_name(
    ...
    *,
    ...
    blending_percent = int(blending_factor * 100.0 + 0.5)
    warping_percent = int(warping_factor * 100.0 + 0.5)
    return (
    ...
    with extractor:
        ...
        for job in progress:
            ...
            if pair_key != current_pair:
                image1 = _load_image(job.image1)
                image2 = _load_image(job.image2)
            ...
            result = morph_with_landmarks(
                ...
                add_border_points=args.add_border_points,
